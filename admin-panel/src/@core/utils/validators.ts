...
export const emailValidator = (value: unknown) => {
  ...
  if (Array.isArray(value))
    return value.every(val => re.test(String(val))) || 'Geçerli bir Email olmalıdır'

  return re.test(String(value)) || 'Geçerli bir Email olmalıdır'
}
...
export const phoneValidator = (value: unknown) => {
  ...
  const re = /^(((\+|00)?(90)|0)[-| ]?)?((5\d{2})[-| ]?(\d{3})[-| ]?(\d{2})[-| ]?(\d{2}))$/

  if (Array.isArray(value))
    return value.every(val => re.test(String(val))) || 'Geçerli bir Telefon Numarası olmalıdır'

  return re.test(String(value)) || 'Geçerli bir Telefon Numarası olmalıdır'
}
